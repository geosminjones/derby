#!/usr/bin/env python3
"""
timer_tab.py - Timer tab and related dialogs for Derby GUI (PyQt6 version)
"""
...
from jframes import SessionList, StoppedSessionList, batch_update, get_dropdown_arrow_path, MessageBox
...
class TimerTab:
    ...
    def _build_ui(self):
        ...
        # Stopped sessions list (recently stopped, quick restart)
        self.stopped_session_list = StoppedSessionList(
            top_frame,
            max_cards=5,
            on_play=self._on_play_stopped_session
        )
        top_layout.addWidget(self.stopped_session_list)
        ...
        # Stopped background tasks list (recently stopped, quick restart)
        self.bg_stopped_session_list = StoppedSessionList(
            bottom_frame,
            max_cards=5,
            on_play=self._on_play_stopped_session
        )
        bottom_layout.addWidget(self.bg_stopped_session_list)
    ...
    def _on_stop_session(self, session_id: str):
        ...
        if session and session.end_time is None:
            ...
            if is_bg:
                self.bg_stopped_session_list.add_stopped_session(
                    project_name=project_name,
                    stop_date=stop_date_str,
                    duration=duration_str
                )
            else:
                self.stopped_session_list.add_stopped_session(
                    project_name=project_name,
                    stop_date=stop_date_str,
                    duration=duration_str
                )
    ...
    def _on_play_stopped_session(self, project_name: str):
        ...
        self.stopped_session_list.remove_card(project_name)
        self.bg_stopped_session_list.remove_card(project_name)
