#!/usr/bin/env python3
"""
tab_summary.py - Summary tab for time aggregations in Derby GUI (PyQt version)
"""
...
from jframes import CTkTable, batch_update, get_dropdown_arrow_path
...
class SummaryTab:
    ...
    def _build_ui(self):
        ...
        period_row.addWidget(self.period_group)
        ...
        tag_label.setStyleSheet(f"color: {colors['text_primary']};")
        tag_row.addWidget(tag_label)

        self.tag_combo = QComboBox()
        self.tag_combo.setFixedSize(120, 28)
        self.tag_combo.setFont(QFont(FONT_FAMILY, 10))
        ...
        tag_row.addStretch()
        period_layout.addLayout(tag_row)
    ...
    def _on_tag_change(self, value: str):
    ...
    def _initialize_tables(self):
        ...
        column_border_mode = db.get_setting("column_border_mode", "none")

        # Project tables - text_time_boundary=2 (after Tags column at index 2)
        # border_after_first=True (border after Project column)
        # border_before_last=True (border before Total/Hours column)
        self.table_standard = CTkTable(
            ...
            text_time_boundary=2,
            border_after_first=True,
            border_before_last=True
        )
        ...
        # Background tables - text_time_boundary=0 (after Task column at index 0)
        # border_after_first=False (not needed, text_time_boundary=0 handles it)
        # border_before_last=True (border before Hours/Total column)
        # Standard view table
        self.bg_table_standard = CTkTable(
            ...
            show_dividers=show_row_dividers,
            column_border_mode=column_border_mode,
            text_time_boundary=0,
            border_after_first=False,
            border_before_last=True
        )
    ...
    def _update_table_divider_settings(self):
        """Update show_dividers and column_border_mode settings on all tables."""
        show_row_dividers = db.get_setting("show_row_dividers", "1") == "1"
        column_border_mode = db.get_setting("column_border_mode", "none")
        if self._tables_initialized:
            ...
            self.table_standard.column_border_mode = column_border_mode
            self.table_weekly.column_border_mode = column_border_mode
            self.table_monthly.column_border_mode = column_border_mode
            self.bg_table_standard.column_border_mode = column_border_mode
            self.bg_table_weekly.column_border_mode = column_border_mode
            self.bg_table_monthly.column_border_mode = column_border_mode
    ...
    def _refresh_standard_inner(self, start_date, end_date, sort_by, group_by):
        ...
        # Get tag filter
        tag = None if self.tag_var == "All" else self.tag_var

        if sort_by == "priority":
            # Get project summary (regular projects only)
            project_summary = db.get_summary_with_priority(start_date=start_date, end_date=end_date, is_background=False, tag=tag)
            ...
            tag_summary = db.get_summary_by_tag(start_date=start_date, end_date=end_date, tag=tag)
    ...
    def _refresh_weekly_inner(self, start_date: datetime, end_date: datetime, sort_by, group_by):
        ...
        if sort_by == "priority":
            ...
            project_summary = db.get_summary_by_day(start_date=start_date, end_date=end_date, is_background=False, tag=tag)
