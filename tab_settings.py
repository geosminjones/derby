#!/usr/bin/env python3
"""
tab_settings.py - Settings tab for Derby GUI (includes appearance and data storage)
PyQt6 version
"""
...
class AppearanceTab:
    ...
    def _build_ui(self):
        ...
        # Column borders section
        column_borders_label = QLabel("Column borders:")
        column_borders_label.setFont(QFont(FONT_FAMILY, 11))
        column_borders_label.setStyleSheet(f"color: {colors['text_secondary']}; background: transparent;")
        display_layout.addWidget(column_borders_label)

        # Radio button group for column border modes
        self.column_border_group = QButtonGroup(self.parent)
        self.column_border_radios = {}

        current_mode = db.get_setting("column_border_mode", "none")

        border_options = [
            ("none", "None"),
            ("text_time", "Between text and time"),
            ("all", "Between every column")
        ]

        for mode_value, mode_label in border_options:
            ...
            """)
            radio.setChecked(mode_value == current_mode)
            radio.toggled.connect(lambda checked, mode=mode_value: self._on_column_border_change(mode, checked))
            self.column_border_group.addButton(radio)
            self.column_border_radios[mode_value] = radio
    ...
    def _on_row_dividers_change(self, state):
        ...
        if hasattr(self.app, 'tab_summary') and hasattr(self.app.tab_summary, '_tables_initialized') and self.app.tab_summary._tables_initialized:
            self.app.tab_summary.refresh()
    ...
    def _on_column_border_change(self, mode: str, checked: bool):
        """Handle column border mode change."""
        if checked:
            db.set_setting("column_border_mode", mode)
            if hasattr(self.app, 'tab_summary') and hasattr(self.app.tab_summary, '_tables_initialized') and self.app.tab_summary._tables_initialized:
                self.app.tab_summary.refresh()
    ...
    def refresh(self):
        ...
        # Update column border radio selection
        current_border_mode = db.get_setting("column_border_mode", "none")
        if current_border_mode in self.column_border_radios:
            self.column_border_radios[current_border_mode].setChecked(True)
